...
ob_start();
	...
	{
	$sql = "SELECT * FROM batch WHERE batchCode='$batch'";
	...
	if(isset($_POST["btnPrint"]))
	{
		...
		$sql = "UPDATE batch SET dateAward='$dateAward' WHERE batchCode='$batch'"; 
  		$result=mysqli_query($con,$sql) or die(mysqli_error($con));
		header("location:rpt_print_certi.php?id1=".$batch );
	}
    ...
    <div class="testbox">
      <form name="printIDForm" method="post" action="#">
		...
		<div class="item">
		 ...
		 <div class="item"><label for="number">Date Award</label>
		<input type="date" id="name" name="dateAward"  required="required" value="<?php echo $dateAward; ?>"/></br></div>
